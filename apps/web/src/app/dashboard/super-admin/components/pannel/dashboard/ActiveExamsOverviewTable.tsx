...
import { Download, Eye, ChevronLeft, ChevronRight, ChevronDown } from "lucide-react";
import { activeExamsOverviewData } from "./mockData";

export function ActiveExamsOverviewTable() {
  return (
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-slate-100 text-xs sm:text-sm font-semibold text-slate-700">
            {activeExamsOverviewData.map((row) => (
              <tr key={row.id} className="hover:bg-slate-50/80 transition-colors">
                ...
                <td className="py-3.5 px-3 text-slate-900 font-bold">{row.participants}</td>
                <td className="py-3.5 px-3 text-slate-900 font-bold">{row.attempts}</td>
