...
export function ExamLiveWorkspace({
  ...
  return (
    <div className="h-dvh w-full max-w-full bg-[#F8FAFC] text-slate-900 flex flex-col font-sans antialiased select-none overflow-x-clip overflow-y-auto box-border">
      <header className="bg-white border-b border-slate-200 sticky top-0 z-30 w-full max-w-full min-w-0 shrink-0 overflow-x-clip">
        <div className="w-full max-w-full min-w-0 box-border px-3 sm:px-4 py-2 flex items-center gap-2 sm:gap-3">
          {/* Left: logo + title + total time */}
          <div className="flex items-center gap-2 min-w-0 flex-1">
            <div className="size-8 rounded-lg bg-gradient-to-br from-violet-600 to-indigo-600 text-white flex items-center justify-center font-black text-[11px] shrink-0">
              IMO
            </div>
            <div className="min-w-0 sm:block">
              <h1 className="text-xs sm:text-sm font-black text-slate-900 leading-tight truncate max-w-[8rem] sm:max-w-[14rem] md:max-w-[18rem]">
                {exam.title || "IMO Olympiad"}
              </h1>
              <p className="text-[10px] font-semibold text-slate-500 truncate">
                Q{currentQIndex + 1}/{TOTAL_QUESTIONS} • {exam.totalMarks || 100} Marks
              </p>
            </div>

            <div className="flex items-center gap-1 px-2 py-1 rounded-lg bg-violet-50 border border-violet-100 text-violet-800 shrink-0">
              <svg className="w-3 h-3 text-violet-600 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2">
                <circle cx="12" cy="12" r="10" />
                <polyline points="12 6 12 12 16 14" />
              </svg>
              <span className="text-[10px] font-bold text-violet-500 hidden sm:inline">Total</span>
              <span className="font-mono text-[11px] sm:text-xs font-black tabular-nums">
                {formatTimer(overallSecondsRemaining)}
              </span>
            </div>
          </div>

          {/* Right: compact question timer + camera */}
          <div className="flex items-center gap-2 shrink-0 min-w-0">
            <div
              className={`flex items-center gap-1.5 px-2 py-1 rounded-lg border min-w-0 ${
                questionTimeLeft <= 10
                  ? "bg-rose-50 border-rose-200 text-rose-700"
                  : "bg-slate-50 border-slate-200 text-slate-800"
              }`}
            >
              <span className="text-[10px] font-bold uppercase tracking-wide text-slate-500 hidden md:inline">
                Q{currentQIndex + 1}
              </span>
              <div className="w-12 sm:w-16 h-1 rounded-full bg-slate-200 overflow-hidden hidden sm:block">
                <div
                  className={`h-full ${questionTimeLeft <= 10 ? "bg-rose-500" : "bg-violet-600"}`}
                  style={{ width: `${(questionTimeLeft / SECONDS_PER_QUESTION) * 100}%` }}
                />
              </div>
              <span
                className={`font-mono text-[11px] sm:text-xs font-black tabular-nums ${
                  questionTimeLeft <= 10 ? "text-rose-600 animate-pulse" : "text-violet-800"
                }`}
              >
        ...
        <div className="w-full h-0.5 bg-slate-100 overflow-hidden">
          <div
            className={`h-full transition-all duration-1000 ${
              questionTimeLeft <= 10 ? "bg-rose-500" : "bg-violet-600"
            }`}
            style={{ width: `${(questionTimeLeft / SECONDS_PER_QUESTION) * 100}%` }}
          />
        </div>
      ...
      {fullscreenWarning && !isTerminated && (
        <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white border-2 border-rose-500 rounded-2xl p-5 sm:p-6 max-w-sm w-full text-center space-y-3 shadow-2xl">
            <div className="size-12 rounded-full bg-rose-100 text-rose-600 flex items-center justify-center text-2xl mx-auto">
              ⚠️
            </div>
            <h3 className="text-base font-black text-slate-900">Full Screen Required</h3>
            <p className="text-xs text-slate-600 leading-relaxed">
              This exam must be completed in full screen mode.
            </p>
            <button
              type="button"
              onClick={ensureFullscreen}
              className="w-full py-2.5 rounded-xl bg-violet-600 text-white font-bold text-sm cursor-pointer"
            >
      ...
      {/* Focused question surface — no nested card */}
      <main
        className={`flex-1 w-full max-w-full min-w-0 box-border overflow-x-clip px-4 sm:px-6 md:px-8 py-4 sm:py-5 ${
          isTerminated ? "pointer-events-none opacity-40 select-none" : ""
        }`}
      >
        {currentQ ? (
          <div className="w-full max-w-3xl mx-auto min-w-0 space-y-4">
            <div className="flex flex-wrap items-center gap-1.5 min-w-0">
              <span className="px-2 py-0.5 rounded-md bg-violet-50 text-violet-700 text-[10px] font-black">
                Q{currentQIndex + 1}/{TOTAL_QUESTIONS}
              </span>
              <span className="px-2 py-0.5 rounded-md bg-slate-100 text-slate-600 text-[10px] font-bold truncate max-w-[12rem]">
            ...
            <p className="text-[13px] sm:text-sm font-semibold text-slate-900 leading-relaxed whitespace-pre-wrap break-words [overflow-wrap:anywhere]">
              {currentQ.question}
            </p>
              ...
              {currentQ.options.map((opt) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full max-w-full min-w-0 text-left px-3 py-2.5 rounded-xl border transition-all flex items-center gap-2.5 cursor-pointer box-border ${
                      isSelected
                        ? "bg-violet-50 border-violet-500 text-violet-950 ring-1 ring-violet-400/30"
                        : "bg-white border-slate-200 text-slate-700 hover:border-violet-300"
                    }`}
                  >
                    <span
                      className={`size-7 rounded-md text-[11px] font-black flex items-center justify-center shrink-0 border ${
                        isSelected
                    ...
                    <span className="text-xs sm:text-[13px] font-medium flex-1 min-w-0 leading-snug break-words [overflow-wrap:anywhere]">
                      {opt.text}
                    </span>
                  </button>
                );
            ...
            <div className="pt-3 flex flex-wrap items-center justify-between gap-2">
              ...
              <p className="text-[10px] text-slate-400 font-medium">
                Auto-saved • Advances every 1 min
              </p>
            </div>
          </div>
        ) : (
          <div className="text-center py-12 text-slate-400 text-sm">Loading question...</div>
        )}
      </main>
    </div>
  );
