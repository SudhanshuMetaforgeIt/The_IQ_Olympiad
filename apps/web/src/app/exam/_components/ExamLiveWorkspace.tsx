...
export function ExamLiveWorkspace({
  ...
  return (
    <div className="min-h-screen w-full bg-[#F8FAFC] text-slate-900 flex flex-col font-sans antialiased select-none overflow-x-hidden">
      {/* Top Fixed Header Bar: Integrated Question Timer, Total Time, and Live Proctoring */}
      <header className="bg-white border-b border-slate-200/90 sticky top-0 z-30 shadow-xs w-full">
        <div className="w-full px-4 sm:px-8 lg:px-12 py-3 flex flex-wrap items-center justify-between gap-3 sm:gap-4">
          {/* Left: Olympiad Title & Metadata */}
          <div className="flex items-center gap-3 shrink-0">
            ...
            <div>
              ...
              <p className="text-xs font-semibold text-slate-500">
                Question {currentQIndex + 1} of {TOTAL_QUESTIONS} • {exam.totalMarks || 100} Marks Total
              </p>
            </div>
          </div>

          {/* Center: Dedicated 1-Minute Question Timer (Fixed in Top Bar) */}
          <div className={`flex items-center gap-2.5 sm:gap-3 px-3 sm:px-4 py-1.5 sm:py-2 rounded-2xl border transition-all ${questionTimeLeft <= 10
              ? "bg-rose-50/90 border-rose-300 shadow-sm shadow-rose-500/10 ring-2 ring-rose-500/20"
              : "bg-slate-50 border-slate-200/90 shadow-2xs"
            }`}>
            <div className={`size-8 sm:size-9 rounded-xl flex items-center justify-center font-bold text-xs shrink-0 border transition-all ${questionTimeLeft <= 10
            ...
            <div className="flex flex-col">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] px-2 py-0.5 rounded-full bg-white text-slate-600 font-bold border border-slate-200/80">
                  1m Limit
                </span>
              </div>

              {/* Progress Bar */}
              <div className="w-28 sm:w-40 h-1.5 rounded-full bg-slate-200/90 overflow-hidden mt-1">
                <div
                  className={`h-full transition-all duration-1000 ${questionTimeLeft <= 10
                      ? "bg-rose-500"
            ...
            {/* Countdown Badge */}
            <div className={`px-2.5 sm:px-3 py-1 rounded-xl border flex items-center gap-1.5 shrink-0 transition-all ${questionTimeLeft <= 10
                ? "bg-rose-100/90 border-rose-300 text-rose-600 animate-pulse font-black"
                : "bg-white border-violet-200 text-violet-800 font-black"
              }`}>
          ...
          {/* Right: Total Exam Time & Mini Live Proctoring */}
          <div className="flex items-center gap-2.5 sm:gap-4 shrink-0">
            <div className="flex items-center gap-2 px-3 sm:px-3.5 py-1.5 sm:py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-700 text-xs font-bold shadow-2xs">
              <svg className="w-3.5 sm:w-4 h-3.5 sm:h-4 text-violet-600 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2">
                <circle cx="12" cy="12" r="10" />
                <polyline points="12 6 12 12 16 14" />
              </svg>
              <span className="text-slate-500 font-medium hidden sm:inline">Total Time:</span>
        ...
        {/* Dynamic Countdown Progress Bar running along the bottom edge of sticky header */}
      ...
      {/* Main Workspace: Full width across screen, left-aligned, no horizontal scroll */}
      <main className={`flex-1 w-full px-4 sm:px-8 lg:px-12 py-5 sm:py-6 flex flex-col ${isTerminated ? "pointer-events-none opacity-40 select-none" : ""}`}>
        {/* Question Card: Full-width, starting from the left side */}
        <div className="w-full bg-white rounded-3xl border border-slate-200/90 p-6 sm:p-8 lg:p-10 flex flex-col justify-between space-y-6 sm:space-y-8 shadow-sm">
          {currentQ ? (
            <div className="space-y-6 text-left">
              ...
              {/* Question Text: Left-aligned, full width, wrapping without horizontal scroll */}
              <div className="text-lg sm:text-xl font-bold text-slate-900 leading-relaxed whitespace-pre-line break-words text-left">
                {currentQ.question}
              </div>
                ...
                {currentQ.options.map((opt) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left p-4 sm:p-4.5 rounded-2xl border-2 transition-all flex items-center gap-4 cursor-pointer transform active:scale-[0.99] ${isSelected
                    ...
                    >
                      <div className={`size-9 rounded-xl font-black text-xs flex items-center justify-center shrink-0 border transition-all ${isSelected
                          ? "bg-violet-600 text-white border-violet-600 shadow-sm shadow-violet-600/30"
                          : "bg-slate-100 text-slate-600 border-slate-200"
                        }`}>
                        {opt.id}
                      </div>
                      <span className="text-sm sm:text-base font-semibold flex-1 leading-snug break-words">
                        {opt.text}
                      </span>
                      {isSelected && (
          ...
          {/* Bottom Bar: Clear Response button & Auto-advance notice */}
          <div className="pt-6 border-t border-slate-100 flex flex-wrap items-center justify-between gap-4 text-xs">
            ...
            <button
              ...
              className={`px-4 py-2.5 rounded-xl border font-bold transition flex items-center gap-1.5 cursor-pointer ${currentAnswer
